...
#' @param multi Logical indicating if the data was generated by a multi-channel system (TRUE) or a one-channel system (FALSE)
#'
#' @return
...
pulse_interpolate <- function(split_window, interpolation_freq = 40, multi) {
	...
	if (interpolation_freq != 0) {
		...
		if (multi) {
			...
			new_split_window <- suppressWarnings(stats::approx(new_split_window$time, new_split_window$val, time2)) %>%
				tibble::as_tibble() %>%
				dplyr::rename(time = x, val = y)
		}
	}
...
pulse_smooth <- function(split_window, bandwidth = 0.2) {
	...
	if (bandwidth != 0) {
		...
		new_split_window <- new_split_window %>%
			dplyr::select(-time) %>%
			dplyr::reframe(dplyr::across(dplyr::everything(), ~stats::ksmooth(time1, .x, kernel = "normal", bandwidth = bandwidth)$y)) %>%
...
pulse_smooth1 <- function(split_window_one_channel, bandwidth = 0.2) {
	stats::ksmooth(
		split_window_one_channel$time,
		split_window_one_channel$val, kernel = "normal",
...
pulse_optimize <- function(pulse_data_split, interpolation_freq = 40, bandwidth = 0.2, raw_v_smoothed = FALSE, multi) {
	## CHECKS INITIATED ## ------------------- ##
