...
#'
#' Note also that even if two PULSE systems have been used in the same *scientific experiment*, data from each device must be processed independently, and only merged at the end. There's no drawback in doing so, it just is important to understand that that's how data must be processed by the [`pulse-package`].
#'
...
#' * [`pulse_read()`] describes constraints to the type of files that can be read with the [`pulse-package`].
...
#'
#' Also check [`pulse_summarise()`] for important info about oversampling and strategies to handle it while processing PULSE data with the [`pulse-package`].
#'
#' @return
...
#' @export
#'
#' @seealso [approx()], [ksmooth()], [progressr::handlers()], [future::plan()]
#'
#' [pulse_read()], [pulse_split()], [pulse_optimize()], [pulse_heart()], [pulse_summarise()]
#'
#' @examples
#' # Begin prepare data ----
